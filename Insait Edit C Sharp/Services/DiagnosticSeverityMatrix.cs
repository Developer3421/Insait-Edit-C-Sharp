...
// ═══════════════════════════════════════════════════════════════════════════════
//  Built-in IDE Analyzer Provider
//  Discovers and caches DiagnosticAnalyzer instances from Roslyn feature
//  assemblies that are already loaded into the process (MEF host).
//  This is what produces IDE0001 … IDE1006 diagnostics.
// ═══════════════════════════════════════════════════════════════════════════════

/// <summary>
/// Discovers and caches the built-in Roslyn IDE DiagnosticAnalyzers from
/// <c>Microsoft.CodeAnalysis.CSharp.Features</c> and related assemblies.
///
/// These analyzers produce Info-level IDE0xxx diagnostics (simplify name,
/// remove unnecessary cast, use pattern matching, etc.) that the compiler
/// alone does not emit.
/// </summary>
public static class BuiltInAnalyzerProvider
{
    ...
    /// <summary>
    /// Returns all built-in C# DiagnosticAnalyzers discovered from currently
    /// loaded assemblies.  The result is cached after the first call.
    /// </summary>
    public static ImmutableArray<DiagnosticAnalyzer> GetCSharpAnalyzers()
    {
        ...
        lock (_lock)
        {
            ...
            // Target assemblies that carry IDE diagnostics
            var targetNames = new[]
            {
            ...
            {
                try { Assembly.Load(name); } catch { /* best-effort */ }
            }
            ...
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                try
                {
                    // Skip obviously irrelevant assemblies to speed up scanning
                    ...
                    foreach (var type in types)
                    {
                        ...
                        catch { /* skip broken analyzer */ }
                    }
                }
                catch { /* skip broken assembly */ }
            }
    ...
    /// <summary>
    /// Merges <paramref name="projectAnalyzers"/> (from NuGet) with the built-in
    /// IDE analyzers, deduplicating by type name.
    /// </summary>
        ...
        IEnumerable<DiagnosticAnalyzer> projectAnalyzers)
    {
        ...
        // Project analyzers take precedence (overwrite built-ins with same type name)
